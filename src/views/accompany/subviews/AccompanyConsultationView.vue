...
onMounted(() => {
  if (AichatStore.accompanyConsultationHistory != "") {
    AichatStore.processRecordsConsultation(AichatStore.accompanyConsultationHistory, textList)
  }
})
...
const sendMessage = async () => {
  if (sms.value != "") {
    ...
    let res;
    try {
      res = await AiConsultantControllerService.chatUsingPost1({
        chatId: UserInfoStore.userinfo.chatConsultationID,
        userInput: sendmsg
      })
      if (res.code == 0) {
        textList.value.push({
